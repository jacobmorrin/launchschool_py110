...
#   Jacob: Create a dictionary to tally letters for each word. 
# 
# Compare values for each key and add up the differences. 
# 
# 
# 
# Make sure I don't miss any keys, and don't double count any either.
...
def anagram_difference(string1, string2):
    ...
    for key in s1_dict:
        difference += abs(s1_dict.get(key) - s2_dict.get(key, 0))

    for key in s2_dict:
        if s1_dict.get(key):
            continue
        else:
            difference += abs(s2_dict.get(key) - s1_dict.get(key, 0))
